...
import { Button, Divider, Input, Modal, message, Table } from 'antd';
...
import { pisCofinsNcmsColumns } from '@/pages/app/PisCofins/constants';
...
const Ncms = () => {
  ...
  const {
    state: { ncms, activeNcm, products },
    filterProducts,
    resetState,
  } = usePisCofinsContext();

  const { mutateAsync, isLoading } = useConfirmAllRegimeProducts('monofasico');
  ...
  const confirmAllProducts = async () => {
    const key = 'pis_cofins_message_key';
  ...
  return (
    <>
      <Title>
        <h2>Lista de NCMs</h2>
        <p>
          Confirme todos os produtos como monofásicos ou filtre-os pelo NCM.
        </p>
      </Title>
          ...
          >
            <ModalContent>
              ...
              <p>
                ...
                encontrados pelo sistema presentes nessa lista são monofásicos.
                Caso queira confirmar a natureza de produtos específicos, feche
                esse aviso e selecione um NCM. Caso queria continuar, clique no
                botão <strong>Confirmar envio</strong>.
              </p>
            </ModalContent>
          </Modal>
